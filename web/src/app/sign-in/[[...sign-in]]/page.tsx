...
export default function SignInPage() {
  ...
  // OAuth 콜백 후 세션이 이미 있으면 자동 리다이렉트
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event) => {
      if (event === 'SIGNED_IN') router.replace(redirectUrl);
    });
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-bg px-4">
      ...
      <div className="w-full max-w-sm bg-bg-card border border-border rounded-xl p-6 shadow-xl">
        <h2 className="text-lg font-semibold text-text-heading mb-6 text-center">로그인</h2>
