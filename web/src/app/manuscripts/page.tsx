...
import {
  ...
  Award, TrendingUp, GraduationCap, Calendar, FileText, ChevronRight, ChevronDown,
} from 'lucide-react';
...
function ManuscriptRow({ m, expanded, onToggle, showStage = true }: RowProps) {
                        ...
                        <div>
                          ...
                          <div className="space-y-1">
                            {group.corresponding.map(m => (
                              <ManuscriptRow key={m.id} m={m} expanded={expandedIds.has(m.id)} onToggle={() => toggleExpanded(m.id)} showStage={false} />
                            ))}
                          </div>
                        </div>
                      )}
