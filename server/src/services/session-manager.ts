...
export async function maybeGenerateSummary(channelId: string, minNewMessages: number = 20): Promise<void> {
  ...
  try {
    ...
    if (summaryUsage && channel?.userId) logApiCost(channel.userId, 'gemini-3.5-flash', summaryUsage.promptTokenCount ?? 0, summaryUsage.candidatesTokenCount ?? 0, 'session_summary').catch(() => {});
...
JSON 배열: [{"type": "dict"|"memo", "data": {...}}]`;
    ...
    if (autoExtractUsage) logApiCost('system', 'gemini-3.5-flash', autoExtractUsage.promptTokenCount ?? 0, autoExtractUsage.candidatesTokenCount ?? 0, 'auto_extract_info').catch(() => {});
...
export async function generateSessionTitle(messages: Array<{ role: string; content: string }>, latestMessage: string): Promise<string | null> {
  try {
    ...
    if (titleUsage) logApiCost('system', 'gemini-3.5-flash', titleUsage.promptTokenCount ?? 0, titleUsage.candidatesTokenCount ?? 0, 'session_title').catch(() => {});
