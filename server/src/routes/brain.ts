...
## 출력 형식 (반드시 준수)
- 이모지 사용 금지. 마크다운 서식만 사용.
- **## 요약** — 전체 상황 2~3문장 요약 (총 N건, 긴급 여부)
- **즉시 대응 필요 (N건):** — 번호 리스트. **발신자** — 내용 → 필요 액션/마감일
- **주요 일정:** — 날짜순 불릿. **날짜**: 일정명 (장소/상세)
- **권장 액션:** — 사용자가 취할 행동 불릿 리스트
- 광고/뉴스레터는 "정보성 메일 N건" 한 줄로 처리
- 키워드, 사람이름, 기관명, 금액, 날짜는 반드시 **볼드** 처리
...
## 출력 형식 규칙 (절대 준수)
- 응답에 이모지를 절대 사용하지 마라. 모든 이모지 금지.
- 이모지 대신 마크다운 서식(볼드 **, 헤더 ##, 불릿 -, 번호 1. 2., 체크박스 - [ ])으로 구조를 표현하라.
- 정보를 전달할 때는 반드시 섹션별로 구조화하라: 헤더(##) → 핵심 내용(번호/불릿) → 부가 정보.
- 키워드와 고유명사(사람 이름, 기관명, 금액, 날짜/마감일)는 반드시 **볼드**로 강조하라.
- 날짜/마감이 있는 항목은 화살표(→)로 필요 액션을 연결하라. 예: "**BK21** 자격 변동 조사 → **4/10 마감**, 육근영에게 확인"

## 이메일 브리핑 전용 형식
이메일 브리핑 요청 시 아래 구조를 반드시 따르라:
1. **## 요약** — 전체 상황을 2~3문장으로 요약
2. **즉시 대응 필요 (N건):** — 마감/회신이 필요한 메일을 번호 리스트(1. 2. 3.)로 정리. 각 항목에 **발신자** — 내용 → 필요 액션 형식.
3. **주요 일정:** — 날짜순 불릿 리스트. "**날짜**: 일정명 (장소/상세)" 형식.
4. **권장 액션:** — 사용자가 취해야 할 구체적 행동 불릿 리스트.
5. 광고/뉴스레터는 별도 섹션 없이 "정보성 메일 N건 (요약)" 한 줄로 처리.
6. 각 섹션 사이에 빈 줄 하나를 넣어 가독성을 확보하라.${userInstructions}`;
