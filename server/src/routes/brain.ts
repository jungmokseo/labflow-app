...
## 출력 형식 (반드시 아래 포맷을 그대로 따르라)

📧 이메일 브리핑 — 날짜 (요일) 시간 EST
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
총 N건 신규 | 대응필요 N건 · 일정 N건 · 정보성 N건 · 광고 N건
...
📊 요약
━━━━━━
- 대응필요 N건: **제목1** — 핵심, **제목2** — 핵심.
- 주요 일정: 이벤트 날짜 및 내용.
- 권장 액션: 사용자가 취해야 할 행동 목록.

규칙:
- ⚠️/📝: 핵심 내용 최대 2줄 + 조치사항(→) 1줄. 배경 생략.
- 📰: "**발신자** (시간) — 한 줄" 만.
- 다건 동일 성격은 1줄 묶음. 조치 없는 항목에 → 달지 않음.
- 키워드, 사람이름, 기관명, 금액, 날짜는 반드시 **볼드**.
...
## 출력 형식 규칙 (절대 준수)
- 마크다운 서식(볼드 **, 헤더 ##, 불릿 -, 번호 1. 2.)을 적극 활용하라.
- 키워드, 고유명사(사람 이름, 기관명, 금액, 날짜/마감일)는 반드시 **볼드**로 강조하라.
- 날짜/마감이 있는 항목은 화살표(→)로 필요 액션을 연결하라.
- 정보를 전달할 때는 반드시 섹션별로 구조화하라.
- 각 섹션 사이에 빈 줄을 넣어 가독성을 확보하라.
- 섹션 헤더에는 구분용 이모지 1개를 허용한다 (📧📊⚠️📝📅📰🏫🏢👤🛒 등).
- 본문 내에서는 이모지를 사용하지 마라.

## 이메일 브리핑 전용 형식 (반드시 아래 구조를 따르라)
이메일 브리핑 요청 시 아래 포맷을 그대로 사용하라. 포맷을 임의로 변형하지 마라.

\`\`\`
📧 이메일 브리핑 — YYYY-MM-DD (요일) HH:MM EST
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
총 N건 신규 | 대응필요 N건 · 일정 N건 · 정보성 N건 · 광고 N건
...
압축 규칙:
- ⚠️/📝 항목: 핵심 내용 최대 2줄 + 조치사항(→) 1줄. 배경·스레드 히스토리 생략.
- 📰 항목: "**발신자** (시간) — 한 줄" 만. 꼬리말 생략.
- 다건 동일 성격: "**전자결재 수신참조** (3건): 출장결과, 휴가신청서, 출장신청" 형식으로 1줄 묶음.
- 조치 없는 항목에는 → 달지 않음.
- 기관별로 그룹핑: 연세대학교/링크솔루텍/개인/광고 순서로 분류.${userInstructions}`;
