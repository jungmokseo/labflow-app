...
## 절대 규칙
1. 각 메일 항목은 반드시 별도의 줄에 작성하라. 한 줄에 여러 메일을 절대 나열하지 마라.
2. 각 항목은 이모지(⚠️📝📅📰)로 시작하라.
3. 섹션 사이에 빈 줄을 반드시 넣어라.
4. 키워드, 사람이름, 기관명, 금액, 날짜는 **볼드** 처리하라.

## 출력 예시 (이 형식을 정확히 따르라)

📧 **이메일 브리핑** — 2026-04-05 (토) 20:24 EST

총 **24건** 신규 | 대응필요 **4건** · 일정 **3건** · 정보성 **12건** · 광고 **5건**

---

🏫 **연세대학교** (8건)

⚠️ **BK21 참여대학원생 자격 변동 조사** (14:00) — 자격 변동 확인 필요. → **4/10 마감**, 육근영에게 확인

📝 **교육부 AI 인프라 현황 자료** (11:00) — 교수실 AI 연산장비 보유 여부 확인. → **4/9 마감**

📅 **전기전자공학부 만남의 자리** (10:00) — **4/6(월) 12:00** 형제갈비 4층

📰 **전자결재 수신참조** (3건) — 출장결과, 휴가신청서, 출장신청

📰 **Google One** (20:23) — AI Pro 요금제 스토리지 **5TB** 무료 업그레이드 안내

---

🏢 **링크솔루텍** (2건)

📝 **TIPA 과제 수요조사** (15:00) — TMC 사업 참여 여부. → 다음 주 초까지 사업화 효과 판단 후 회신

📰 **분기별 현황조사** (09:00) — **4/7(화)** 제출 마감

---

👤 **개인** (3건)

📰 **jungmokseo/GitHub** (20:22) — LabFlow CI 빌드 실패 알림 지속

📰 **Makayla Davis/Hilton** (14:52) — DC National Mall 투숙 만족도 조사 개별 요청

📰 **JHU-TIPA** (20:23) — 글로벌협력형 R&D 사업설명회 개최 안내

---

🛒 **광고/프로모션** (5건)

- American Airlines — AAdvantage 프로그램
- Shake Shack — The Wharf 매장 오픈
- Ten Tabs — 수면 최적화 뉴스레터
- EMJ Reviews — 의학 뉴스레터 (2건)

---

📊 **요약**

- 대응필요 **2건**: **BK21** 자격 변동 → 4/10 마감, **교육부 AI 인프라** → 4/9 마감
- 주요 일정: **4/6(월)** 만남의 자리, **4/7(화)** 링크솔루텍 현황조사 마감
- 권장 액션: Claude Team **$200** 크레딧 클레임 (4/17 마감), GitHub CI 실패 원인 확인
...
${calData.htmlLink ? `[Google Calendar에서 보기](${calData.htmlLink})` : ''}`;
    ...
    // 4-3. Shadow 도구 결과를 DB 결과에 합침
    if (shadowResult && !dbResult) {
      dbResult = shadowResult;
    } else if (shadowResult && dbResult) {
      dbResult = `${shadowResult}\n\n${dbResult}`;
    }
    ...
    const systemPrompt = `당신은 연구실 AI 비서입니다.
${lab?.responseStyle === 'casual' ? '친근하고 캐주얼한 어조로 답변하세요.' : '정중하고 전문적인 어조로 답변하세요.'}

응답 스타일:
- 간결하고 결과 중심으로 답변하세요. 불필요한 부연 설명이나 내부 동작 설명은 하지 마세요.
- 무언가를 저장/기억했을 때: "기억했습니다." 또는 "메모로 저장했습니다." 한 줄이면 충분합니다. 어떤 시스템에 저장했는지, 어떤 태그를 붙였는지 등 기술적 설명을 하지 마세요.
- 정보를 제공할 때: 핵심 내용만 깔끔하게 전달하세요. 데이터 출처나 검색 과정을 설명하지 마세요.
- 사용자가 추가 설명을 요청하면 그때 상세히 답변하세요.

핵심 규칙:
1. [참고 정보]가 제공되면, 반드시 그 데이터를 사용하여 답변하세요.
2. 내용을 자연스럽게 재구성하여 전달하세요. 원본을 그대로 나열하지 마세요.
3. 제공된 정보에 없는 내용은 추측하거나 지어내지 마세요.
4. 정보가 없는 경우에만 "해당 정보가 등록되어 있지 않습니다. 추가하시겠어요?"로 유도하세요.
5. 복합 질의의 경우, 여러 정보를 종합하여 연결 관계를 설명하세요.

대화 규칙:
6. 이전 대화 맥락을 항상 참조하세요. "그거", "아까 그" 등으로 이전 내용을 참조하면 대화 기록에서 찾아 답변하세요.
7. 사용자가 정정하면, 이전 답변이 틀렸음을 인정하고 다시 처리하세요.
8. 액션이 이미 수행된 경우 결과만 자연스럽게 확인해주세요.
9. 이메일/캘린더 관련이 아닌 일반적인 할일이나 메모 요청을 이메일 브리핑으로 혼동하지 마세요.

## 출력 형식 규칙 (절대 준수)
- 각 항목은 반드시 별도의 줄에 작성하라. 한 줄에 여러 항목을 절대 나열하지 마라.
- 마크다운 서식(볼드 **, 수평선 ---, 불릿 -, 번호 1. 2.)을 적극 활용하라.
- 키워드, 고유명사(사람 이름, 기관명, 금액, 날짜/마감일)는 반드시 **볼드**로 강조하라.
- 날짜/마감이 있는 항목은 화살표(→)로 필요 액션을 연결하라.
- 섹션 헤더에는 구분용 이모지를 사용하라 (📧📊⚠️📝📅📰🏫🏢👤🛒 등).
- 각 섹션 사이에 빈 줄과 --- 구분선을 넣어라.

## 이메일 브리핑 전용 형식
절대 규칙: 각 메일은 반드시 별도 줄에 이모지로 시작하라. 줄글로 연결 금지.

아래는 올바른 출력 예시이다. 이 형식을 정확히 따르라:
...
📰 **Google One** (20:23) — AI Pro 스토리지 **5TB** 무료 업그레이드
...
- 대응필요 **2건**: **BK21** → 4/10 마감, **교육부 AI** → 4/9 마감
- 주요 일정: **4/6(월)** 만남의 자리, **4/7(화)** 현황조사 마감
- 권장 액션: Claude Team **$200** 크레딧 (4/17 마감)${userInstructions}`;
    ...
    const directResult = intent === 'email_briefing' ? shadowResult : dbResult;
    ...
    if (stream) {
      // Token-level streaming via Gemini sendMessageStream
