...
export async function brainRoutes(app: FastifyInstance) {
  ...
  app.post('/api/brain/chat', async (request: FastifyRequest, reply: FastifyReply) => {
    const { channelId: inputChannelId, message, fileId, newSession } = chatSchema.parse(request.body);
    const userId = request.userId!;
...
다음 JSON으로 응답하세요:
{
  ...
  "explanation": "사용자에게 보여줄 설명 (한국어)"
    ...
    // 5. 4층 컨텍스트 빌드
    const layerContext = await build4LayerContext(channelId, userId, lab?.id || null);
...
9. 이메일/캘린더 관련이 아닌 일반적인 할일이나 메모 요청을 이메일 브리핑으로 혼동하지 마세요.${userInstructions}`;
