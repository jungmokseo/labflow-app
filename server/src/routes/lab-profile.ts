...
themes는 3~5개, keywords는 테마별 3~6개. 한글+영문 혼용.`;
        ...
        if (onboardingUsage) logApiCost(request.userId!, 'gemini-3.5-flash', onboardingUsage.promptTokenCount ?? 0, onboardingUsage.candidatesTokenCount ?? 0, 'lab_onboarding').catch(() => {});
...
- 찾을 수 없는 카테고리는 빈 배열로`;
    ...
    try {
      ...
      if (homepageUsage) logApiCost(request.userId!, 'gemini-3.5-flash', homepageUsage.promptTokenCount ?? 0, homepageUsage.candidatesTokenCount ?? 0, 'lab_homepage_extract').catch(() => {});
      const text = result.response.text().trim();
