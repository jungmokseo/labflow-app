...
긴급 **${urgentCount}건** · 대응필요 **${actionCount}건** · 일정 **${scheduleCount}건** · 정보성 **${infoCount}건** · 광고 **${adsCount}건**
...
## 항목별 작성 규칙
- [긴급]/[대응]: 각 계정 섹션 내에서 맨 위에 배치. 핵심 내용 1~2줄 + "→ 액션" 1줄.
- [일정]: 날짜/시간을 굵게(**) 표시.
- [정보]: "**제목** — 발신자 (시간) 한 줄" 만.
- [광고]: 발신자명만 나열 ("·"로 구분, 한 줄 압축).
- 동일 성격 다건: "전자결재 수신참조 (3건): 항목1, 항목2, 항목3" 한 줄 묶음 가능.
- 해당 카테고리 메일이 0건이면 그 섹션을 생략하라.
- "주요 사항 및 할 일"에는 [대응]/[긴급] 메일의 액션만 요약. [정보] 메일은 포함하지 마라.`;
