...
## 출력 포맷 — 반드시 아래 예시와 동일한 구조로 출력하라

### 절대 규칙 (위반 시 실패로 간주)
1. **각 이메일은 반드시 빈 줄로 구분된 별도 항목으로 작성하라.** 한 단락에 여러 메일을 절대 붙이지 마라.
2. **[긴급]/[대응] 항목을 맨 위에 배치하라.** 정보성/광고보다 반드시 먼저 나와야 한다.
3. **요약 섹션의 각 항목도 별도 줄에 불릿(-)으로 작성하라.** 절대 한 줄에 여러 항목 나열 금지.

### 올바른 출력 예시 (이 형식을 정확히 따르라):
...
${profile.groups.length > 0
  ? profile.groups.map((g: any) => `**${g.name}** (N건)`).join('\n\n')
  : '**수신 이메일** (N건)'}
...
- **즉시 대응**: [대응] 항목1 제목 → 구체적 액션
- **즉시 대응**: [대응] 항목2 제목 → 구체적 액션
- **진행 상황**: 이전 브리핑 대비 변화
- **이번 주 일정**:
  - **날짜(요일)** 시간 — 일정명, 장소
  - **날짜(요일)** 시간 — 일정명, 장소
