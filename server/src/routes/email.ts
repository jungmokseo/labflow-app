...
**핵심 원칙: 메일함을 그대로 나열하지 마라. 각 기관 섹션 안에서 중요도순으로 계층화하라. 대응 필요한 메일은 상세하게, 광고는 제목만.**

반드시 지켜야 할 형식 규칙:
- 줄글(paragraph) 형태로 여러 메일을 묶어 서술하지 마라. 각 메일은 독립된 불릿 항목이다.
- 하나의 불릿(-)에 하나의 메일만 기술하라.
- 각 불릿 항목 사이에 빈 줄을 넣어 시각적으로 구분하라.
- 이모지 절대 사용 금지. 대괄호 라벨과 마크다운 서식만 사용.
- 한국어로 작성. 마크다운 문법. HTML 금지.
- 한국발 메일: KST → ${tzLabel} 병기.
- 같은 스레드의 여러 메일은 타임라인으로 합쳐서 한 항목으로 정리.
...
**[대응 필요]** — 이 섹션에는 회신/결정/승인이 필요한 메일만. 본문을 읽고 액션을 구체적으로 추출.

- [대응] **메일 제목** — 발신자 (HH:MM)
  본문 핵심 2~3줄: 상대방이 원하는 것, 배경 맥락, 마감
  → **액션**: 구체적으로 해야 할 일 (예: "4/15까지 회신 필요")

**[주요 정보]** — 논문/연구/행정/일정/사업 관련. 원문을 안 열어도 될 만큼 충분히 요약.

- [논문] **메일 제목** — 발신자 (HH:MM)
  상태/결과 요약 1~2줄. 논문명, 저널명 명시.
...
**[기타]** — 공지, 설문, CC 등 당장 액션 불필요. 제목+발신자+한 줄만.

- **제목** — 발신자 (HH:MM) 한 줄 요약
- 동일 성격 다건: "전자결재 수신참조 (3건): 항목1, 항목2, 항목3"`).join('\n\n---\n\n')
  : `### 수신 이메일 (N건)
...
위와 동일 계층 구조: [대응 필요] → [주요 정보] → [기타] 순.

---

### 광고/프로모션 (~N건)

**상세 내용 일절 불필요.** 발신자와 제목을 한 줄로 압축 나열.

[발신자: 제목] · [발신자: 제목] · [발신자: 제목] ...
...
- **이번 주 마감:**
  - **날짜(요일)** — 마감 내용
- **진행 상황**: 이전 브리핑 대비 변화 (해당 시)

### 템플릿 끝 ###

## 항목 분류 & 상세도 가이드

**중요도에 따라 상세도가 극적으로 달라져야 한다. 잡다한 메일까지 상세히 쓰면 안 된다:**

| 구분 | 상세도 | 작성법 |
|------|--------|--------|
| [대응 필요] | ★★★ 최고 | 본문 2~3줄 요약 + 맥락 + "→ **액션**" 1줄. 이것만 읽으면 바로 행동 가능하게. |
| [주요 정보] | ★★ 중간 | **딱 1줄.** 핵심 팩트(논문명/결과/마감일)만. 부연 설명 금지. |
| [기타] | ★ 최소 | **제목만.** 요약도 불필요. 동일 유형 3건 이상이면 반드시 한 줄로 묶어라. 예: "학부 공지 (3건): 교수법 특강, SW 주의, 포상 안내" |
| [광고] | 없음 | 발신자만 "·"로 나열. 제목도 불필요. 예: "MRS · Hilton · Costco · Skyscanner" |

**간략화 원칙 (매우 중요):**
- 메일함을 그대로 옮겨놓으면 안 된다. 브리핑은 "30초 스캔"이 목적이다.
- [대응 필요]만 상세하게. 나머지는 최소한으로 압축.
- [기타] 섹션이 5건 이상이면 반드시 유형별로 묶어 3줄 이내로.
- [광고] 섹션은 발신자 나열 한 줄이면 충분. 2줄 이상 금지.
- 전체 브리핑이 너무 길면(30줄 이상) [기타]와 [광고]를 더 압축하라.

- [대응 필요]: 교수님 의견·결정·승인·회신이 필요한 메일, 마감 24시간 이내. 본문을 꼼꼼히 읽고 액션을 추출.
- [주요 정보] 세부 라벨: [논문]/[행정]/[일정]/[사업]/[학생] 중 맥락에 맞게 선택.
- [기타]: CC 공지, 뉴스레터, 설문, 특강 안내 등.
- [광고]: Call for Papers, 컨퍼런스 초대, 프로모션, 마케팅.
- 해당 카테고리 메일이 0건이면 해당 소제목("[대응 필요]" 등)을 생략하라.
- "요약 및 할 일"에는 [대응 필요] 메일의 액션만 포함. 정보성 메일은 포함하지 마라.`;
        ...
        try {
          const response = await anthropic.messages.create({
            ...
            messages: [{
              role: 'user',
              content: `다음 이메일 ${emailDataForPrompt.length}건을 위 템플릿 형식 그대로 브리핑하라.

중요: 줄글로 작성하지 마라. 각 메일을 독립 불릿(-)으로, 빈 줄로 구분하라. 계정(기관)별 섹션을 ### 헤딩으로 나누라.
