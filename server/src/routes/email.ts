...
async function classifyEmailsWithSonnet(
  ...
  try {
    ...
    if (userId) {
      ...
      logApiCost(userId, 'claude-sonnet-4-6', response.usage.input_tokens, response.usage.output_tokens, 'email_classify').catch(() => {});
    }
...
## 중요도 판별 기준 — "나와 직접 관련된 진행 중인 건"

**이 구분이 브리핑의 핵심이다. 아래 기준으로 모든 메일을 3단계로 분류하라:**

### 주요 (불릿으로 상세 표시)
"내가 당사자"인 진행 중인 건:
- 내가 제출/수정 중인 **논문**의 결과, 리비전, 게재 확인
- 내가 참여하는 **연구 과제**의 보고서, 공모, 진행 상황
- **학생**이 나에게 직접 보낸 메일 (보고, 서류, 질문)
- **거래처/파트너**의 회신, 진행 상황 업데이트
- **회신/결정/승인**을 요청받은 메일
- 구체적 **마감일**이 있는 행정 처리
- 확정된 **미팅/일정** (날짜+시간 포함)

### 비주요 (한 줄 압축, 불릿 없이)
"일방적으로 온 것" 또는 "당장 액션 불필요":
- CC로 온 공지, 학과 공지, 학교 공지
- 뉴스레터, 설문, 특강/세미나 안내
- 과제 공모 안내 (내가 지원 예정이 아닌 것)
- 연구재단/기관 일반 안내
- 금융/보험/생활 알림

### 광고 (발신자만 나열)
- Call for Papers, 저널 투고 초대, 학회 등록 홍보
- 컨퍼런스 광고, 프로모션, 마케팅
- 상품 배송/거래명세서
...
${profile.groups.length > 0
  ? profile.groups.map((g: any) => `### ${g.emoji} ${g.name}

**[대응]**

- [대응] **메일 제목** — 발신자 (HH:MM)
  핵심 2~3줄: 상대방 요청, 맥락, 마감
  → **액션**: 구체적 할 일

**[주요]**

- [논문] **제목** — 발신자 (HH:MM): 핵심 1줄
- [과제] **제목** — 발신자 (HH:MM): 핵심 1줄
- [학생] **제목** — 발신자 (HH:MM): 핵심 1줄

**[비주요]**

학과 공지 (3건): 항목1, 항목2, 항목3
제목 — 발신자 (HH:MM)`).join('\n\n---\n\n')
  : `### 수신 이메일
...
## 포맷 규칙 (출력에 이 규칙을 포함시키지 마라)

1. [대응]: 불릿(-) + 핵심 2~3줄 + → 액션. 상세하게 작성.
2. [주요]: 불릿(-) + **제목 — 발신자: 1줄**. 원문 안 열어도 이해되는 수준.
3. [비주요]: **불릿 없이** 제목만 한 줄. 5건 이상이면 유형별 묶음 (예: "학과 공지 (4건): 항목1, 항목2, ...").
4. [광고]: 발신자만 · 구분 1줄 나열. 내용 불필요.
5. 0건인 카테고리는 소제목 자체 생략.
6. "할 일"에는 [대응] 액션만 포함. 정보성 메일 제외.
7. 브리핑은 "30초 스캔"이 목적. 메일함 복사가 아니다.`;
...
${emailDataForPrompt.join('\n\n')}`,
          ...
          logApiCost(userId, 'claude-sonnet-4-6', response.usage.input_tokens, response.usage.output_tokens, 'email_briefing').catch(() => {});
          const textBlock = response.content.find(b => b.type === 'text');
      ...
      for (const task of (extracted.tasks || [])) {
        ...
        const capture = await prisma.capture.create({
          data: {
            ...
            // LLM dueDate가 'ASAP'·'내일' 등 비표준이면 Invalid Date → Prisma throw. isNaN 가드.
            actionDate: (() => { const d = task.dueDate ? new Date(task.dueDate) : null; return d && !isNaN(d.getTime()) ? d : null; })(),
