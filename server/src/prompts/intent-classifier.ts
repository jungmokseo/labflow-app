...
- daily_brief: 오늘 전체 브리핑/우선순위 요청 (이메일+태스크+논문 종합). 예: "today", "오늘 브리핑", "오늘 뭐해야 해?". **주의: "일정"이 포함되면 calendar_query를 선택하라.**
...
- calendar_query: 캘린더/일정 조회 관련. **"일정"이 포함되면 반드시 이것을 선택.** 예: "오늘 일정", "오늘 일정 정리해줘", "이번주 스케줄", "다음 미팅 언제", "일정 알려줘", "오늘 할 일정"
...
- **"오늘 일정 정리해줘" → calendar_query ("일정" 포함 = 캘린더)**, daily_brief가 아님!
- **"오늘 일정 알려줘" → calendar_query**, daily_brief가 아님!
- "오늘 뭐해야 해?" → daily_brief (종합 브리핑, "일정" 미포함)
