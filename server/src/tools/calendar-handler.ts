...
export async function handleCalendarCreate(
  ...
  try {
    ...
    const calModel = genAI.getGenerativeModel({ model: 'gemini-3.5-flash' });
...
메시지: "${message}"`;
    ...
    if (calUsage) logApiCost(userId, 'gemini-3.5-flash', calUsage.promptTokenCount ?? 0, calUsage.candidatesTokenCount ?? 0, 'calendar_extract').catch(() => {});
...
${context}`;
  ...
  if (papersGeminiUsage) logApiCost(userId, 'gemini-3.5-flash', papersGeminiUsage.promptTokenCount ?? 0, papersGeminiUsage.candidatesTokenCount ?? 0, 'papers_tool_fallback').catch(() => {});
