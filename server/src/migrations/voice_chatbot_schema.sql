...
CREATE INDEX IF NOT EXISTS idx_voice_sessions_persona ON voice_sessions (persona_id);
...
CREATE INDEX IF NOT EXISTS idx_voice_turns_session ON voice_turns (session_id, turn_index);
...
CREATE INDEX IF NOT EXISTS idx_corrections_user ON english_corrections (user_id);
CREATE INDEX IF NOT EXISTS idx_corrections_type ON english_corrections (type);
CREATE INDEX IF NOT EXISTS idx_corrections_session ON english_corrections (session_id);
...
RETURNS TABLE (
  id UUID, paper_id TEXT, title TEXT, authors TEXT, abstract TEXT, chunk_text TEXT, similarity FLOAT
)
LANGUAGE plpgsql AS $$
BEGIN
  RETURN QUERY
  SELECT pe.id, pe.paper_id, pe.title, pe.authors, pe.abstract, pe.chunk_text,
...
SELECT persona_id, COUNT(*) AS total_sessions,
  AVG(total_turns) AS avg_turns, AVG(avg_latency_ms) AS avg_latency,
  SUM(total_tool_calls) AS total_tools_used, SUM(error_count) AS total_errors,
  COUNT(CASE WHEN language = 'ko' THEN 1 END) AS korean_sessions,
  COUNT(CASE WHEN language = 'en' THEN 1 END) AS english_sessions
FROM voice_sessions WHERE ended_at IS NOT NULL GROUP BY persona_id;
